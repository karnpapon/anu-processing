...
public class Nodes extends Scene {
  ...
  void hit(HitData data) {
    ...
    newNode.x = random(-maxHorizDim, maxHorizDim) + 550.0;
    newNode.y = random(-maxVertDim, maxVertDim) + 550.0;
    newNode.z = random(-maxHorizDim, maxHorizDim);
    newNode.scale = random(Defaults.baseScale*0.5, Defaults.baseScale);
